...
from omniagentpay.core.types import WalletInfo, WalletSetInfo, AccountType, WalletState, CustodyType
...
async def test_wallet_api():
    ...
    client._wallet_service.create_wallet = MagicMock(return_value=WalletInfo(
        id="w-new", address="0xNew", blockchain="ARC", state=WalletState.LIVE,
        wallet_set_id="ws-mock", custody_type=CustodyType.DEVELOPER, account_type=AccountType.EOA
    ))
    client._wallet_service.create_wallet_set = MagicMock(return_value=WalletSetInfo(
        id="ws-new", name="set-new", custody_type=CustodyType.DEVELOPER,
        create_date="2023-01-01", update_date="2023-01-01"
    ))
    
    # 1. Test Explicit Set Reuse
    print("\n1. Testing create_wallet(wallet_set_id='ws-existing')")
    w = await client.create_wallet(wallet_set_id="ws-existing")
    ...
    w = await client.create_wallet(name="my-set")
