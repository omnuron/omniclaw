...
class BudgetGuard(Guard):
    ...
    async def get_total_spent(self, wallet_id: str) -> Decimal:
        ...
        return await self._get_spent(wallet_id)
    ...
    async def reserve(self, context: PaymentContext) -> str | None:
        ...
        if not self._storage: return None
        ...
        if not period_keys: return None # No limits configured
        ...
        try:
            ...
            for limit_type, key_base in period_keys.items():
                ...
                def _parse_val(d: Any) -> Decimal:
                    ...
                    if isinstance(d, dict): return Decimal(str(d.get("value", "0")))
    ...
    async def commit(self, token: str | None) -> None:
        if not token or not self._storage: return
        import json
        
        try:
            data = json.loads(token)
            if data.get("v") != 2: return
    ...
    async def get_total_spent(self, wallet_id: str) -> Decimal:
        """Get total amount spent."""
        if not self._storage: return Decimal("0")
        key = f"budget:{wallet_id}:{self.name}:total"
        data = await self._storage.get("guard_state", key)
        if isinstance(data, dict): return Decimal(str(data.get("value", "0")))
